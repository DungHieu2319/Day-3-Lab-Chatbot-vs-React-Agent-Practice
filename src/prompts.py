...
_EXAMPLE_TRACE = (
    ...
    "Anh Tú!\n\n"
    "--- Ví dụ 2: người nhận KHÔNG có tên riêng, chỉ được mô tả tự do ---\n"
    "Question: Bạn gái mình thích sáng tạo, hay vẽ vời, sống nội tâm. Ngân\n"
    "sách 500000đ.\n"
    "Thought: Không có tên riêng để tra get_personality_profile (tool này chỉ\n"
    "có dữ liệu của người đã làm trắc nghiệm trong hệ thống). Người dùng đã tự\n"
    "mô tả sở thích rồi nên rút từ khóa 'vẽ tranh, sáng tạo' và tìm quà luôn.\n"
    'Action: search_gift_catalog[{"interests": "vẽ tranh, sáng tạo", "budget": 500000}]\n'
    "Observation: (hệ thống chèn kết quả thật vào đây)\n"
    "Thought: Có danh sách rồi, kiểm tra tồn kho món phù hợp nhất trước khi\n"
    "chốt.\n"
    'Action: check_gift_availability[{"gift_id": "GIFT_001"}]\n'
    "Observation: (hệ thống chèn kết quả thật vào đây)\n"
    "Thought: Còn hàng, đủ căn cứ trả lời.\n"
    "Final Answer: (nêu đúng tên, giá, mã sản phẩm lấy từ Observation)"
)
...
_BUSINESS_FLOW_GUARDRAILS = (
    ...
    "════════════════════════════════════\n"
    "1. Nếu người dùng nhắc một NGƯỜI CỤ THỂ cần chọn quà (có tên/username, vd\n"
    "   'Anh Tú', 'minh_anh') → LUÔN gọi get_personality_profile ĐẦU TIÊN trước\n"
    "   bất kỳ tool nào khác. Nếu tool báo lỗi không tìm thấy: KHÔNG bịa ra tính\n"
    "   cách. Hãy hỏi lại người dùng mô tả trực tiếp sở thích/tính cách của người\n"
    "   nhận, rồi dùng thẳng search_gift_catalog với thông tin đó.\n\n"
    "1b. Nếu người dùng KHÔNG nêu tên riêng mà TỰ MÔ TẢ tính cách/sở thích bằng\n"
    "   lời của mình (vd 'thích sáng tạo, hay vẽ vời, sống nội tâm') → KHÔNG gọi\n"
    "   get_personality_profile (người này không có trong hệ thống trắc nghiệm,\n"
    "   tool chắc chắn báo lỗi không tìm thấy, gọi chỉ tốn 1 bước vô ích). Thay\n"
    "   vào đó, rút ra các từ khóa sở thích trực tiếp từ mô tả của người dùng và\n"
    "   gọi thẳng search_gift_catalog(interests=..., budget=...). Chỉ dùng\n"
    "   suggest_gift_by_personality khi người dùng nêu đúng MỘT trong 4 nhãn\n"
    "   tính cách chuẩn (Người Sáng Tạo / Người Phiêu Lưu / Người Phân Tích /\n"
    "   Người Kết Nối) chứ không phải mô tả tự do — vì tool này chỉ nhận\n"
    "   personality_type là nhãn cố định, không suy luận được từ câu mô tả dài.\n\n"
    ...
    "   LƯU Ý QUAN TRỌNG: loai_tru chỉ so khớp từ khóa với TÊN sản phẩm (vd\n"
    "   'đồng hồ' sẽ loại các sản phẩm có chữ 'đồng hồ' trong tên) — nó KHÔNG\n"
    "   hiểu được các điều kiêng kỵ trừu tượng như 'số lượng 4 hoặc 9' hay ý\n"
    "   nghĩa văn hóa của một món đồ không nêu rõ trong tên. Vì vậy sau khi có\n"
    "   Observation từ search_gift_catalog, bạn PHẢI tự rà lại danh sách kết\n"
    "   quả trả về so với đầy đủ các mục 'nên tránh' đã tra được — nếu có sản\n"
    "   phẩm nào (dù không bị loai_tru chặn) vẫn vi phạm điều kiêng kỵ về ý\n"
    "   nghĩa/số lượng, hãy tự loại nó khỏi gợi ý cuối cùng và nêu lý do ngắn\n"
    "   gọn cho người dùng, không chỉ dựa hoàn toàn vào việc tool đã lọc hộ.\n\n"
...
_RECOVERY_GUARDRAILS = (
    ...
    "- Nếu Observation của search_gift_catalog / suggest_gift_by_personality báo\n"
    "  KHÔNG tìm thấy quà phù hợp (ngân sách quá thấp hoặc sở thích quá hẹp):\n"
    "  KHÔNG gọi lại y hệt tool đó nhiều lần với cùng tham số để 'thử vận may'.\n"
    "  Tối đa thử lại 1 lần với ngân sách/từ khóa nới rộng hợp lý (vd tăng ngân\n"
    "  sách một khoảng vừa phải, hoặc rút gọn sở thích còn từ khóa cốt lõi\n"
    "  nhất) nếu thấy hợp lý, rồi dừng. Sau đó thành thật báo với người dùng là\n"
    "  chưa tìm được quà phù hợp, và đề xuất hướng đi tiếp theo (vd hỏi người\n"
    "  dùng có sẵn sàng tăng ngân sách không, hoặc hỏi thêm sở thích khác).\n"
    "  TUYỆT ĐỐI KHÔNG tự nêu ra một con số 'giá thấp nhất hiện có trong\n"
    "  catalog' nếu con số đó CHƯA từng xuất hiện trong bất kỳ Observation nào\n"
    "  — không tool nào trả về sẵn giá thấp nhất toàn catalog, nên nêu con số\n"
    "  đó tức là đang bịa. Nếu muốn gợi ý mức giá khả thi, hãy đề xuất người\n"
    "  dùng thử lại với một mức ngân sách cụ thể cao hơn, thay vì khẳng định\n"
    "  đó là mức giá thấp nhất thật sự đang có.\n"
